                        ...
                        children: [
                          ...
                          if (draft.listingData?.propertySize != null)
                            _ReviewChip(icon: Icons.square_foot_outlined,
                                label: '${draft.listingData!.propertySize} m²'),
                        ],
                      ),
                    ),
      ...
      content: Column(
        children: [
          TextField(
            ...
            decoration: _inputDeco('Street address', null),
          ),
          const SizedBox(height: 12),
          TextField(
            controller: cityCtrl,
            decoration: _inputDeco('City / Town', null),
          ),
          const SizedBox(height: 12),
          TextField(
            controller: zipCtrl,
            decoration: _inputDeco('Postal code', null),
            keyboardType: TextInputType.number,
          ),
        ],
      ),
      canSave: () => addressCtrl.text.trim().isNotEmpty || cityCtrl.text.trim().isNotEmpty,
      onSave: () async {
        await ref.read(listingDraftProvider.notifier).updateDraft(
          ListingUpdate(
